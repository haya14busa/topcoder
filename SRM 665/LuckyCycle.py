...
def dfs(graph, start, end):
    ...
    while stack:
        ...
        if node == end:
            ...
            visited.append(node)
            stack.extend(map(lambda nnw: (nnw[0], nnw[1] + w), graph[node]))
    return None
...
class LuckyCycle:
    def getEdge(self, edge1, edge2, weight):
        ...
        ws = map(lambda w: (1 if w == 7 else -1), weight)
        for n1, n2, w in zip(edge1, edge2, ws):
            graph[n1].append((n2, w))
            graph[n2].append((n1, w))
        # O(10 ** 5)
        for (start, end) in itertools.combinations(range(1, n + 1), 2):  # 100 C 2 == 4950
            se_w = dfs(graph, start, end)  # O(n) 100
            if se_w and abs(se_w) == 1 and start not in map(lambda x: x[0], graph[end]):
                return (start, end, (4 if se_w > 0 else 7))
        return ()
