...
// CUT begin
	public static void main(String[] args){
		System.err.println("LuckyCycle (500 Points)");
		System.err.println();
	...
	static void runTest(HashSet<Integer> caseSet) {
	    ...
	    while (true) {
            ...
            int[] edge1 = new int[Integer.parseInt(Reader.nextLine())];
            for (int i = 0; i < edge1.length; ++i)
                edge1[i] = Integer.parseInt(Reader.nextLine());
            int[] edge2 = new int[Integer.parseInt(Reader.nextLine())];
            for (int i = 0; i < edge2.length; ++i)
                edge2[i] = Integer.parseInt(Reader.nextLine());
            int[] weight = new int[Integer.parseInt(Reader.nextLine())];
            for (int i = 0; i < weight.length; ++i)
                weight[i] = Integer.parseInt(Reader.nextLine());
            Reader.nextLine();
            ...
            if (doTest(edge1, edge2, weight, __answer))
                passed++;
	    }
	    if (caseSet.size() > 0) cases = caseSet.size();
        System.err.println(String.format("%nPassed : %d/%d cases", passed, cases));

        int T = (int)(System.currentTimeMillis() / 1000) - 1439301618;
        double PT = T / 60.0, TT = 75.0;
        System.err.println(String.format("Time   : %d minutes %d secs%nScore  : %.2f points", T / 60, T % 60, 500 * (0.3 + (0.7 * TT * TT) / (10.0 * PT * PT + TT * TT))));
	}

	static boolean doTest(int[] edge1, int[] edge2, int[] weight, int[] __expected) {
	...
	static class Reader {
        private static final String dataFileName = "LuckyCycle.sample";
